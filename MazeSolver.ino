/* Author: Lisi Useini & Vion Hasaj
   Nutzen: Der Roboter begeht ein Labyrinth, welches aus schwarzen Streifen
           auf weissem Hintergrund besteht.
   Datum: 05.07.2019
*/
...
void setup()
{
  ...
  // mazeSolve() kontrolliert jedes Segment des Labyrinths bis ein Pfad
  // gefunden wurde.
  mazeSolve();
}
...
void mazeSolve()
{
  ...
  // Verzögerung, nachdem der Benutzer auf dem Knopf gedruckt hat, damit der
  // nicht schon unerwartet losfährt.
  delay(1000);
  ...
  while(1)
  {
    ...
    // Geradeaus fahren um zu der ersten Verzweigung anzukommen
    // und dabei auf die Richtungen der Verzweigungen zu achten.
    bool foundLeft, foundStraight, foundRight;
    driveToIntersectionCenter(&foundLeft, &foundStraight, &foundRight);

    if(aboveDarkSpot())
    {
      // Das Ende des Labyrinths wurde gefunden.
      break;
      lcd.print("JUHU");
    }

    // Wahl der Richtungsänderung
    char dir = selectTurn(foundLeft, foundStraight, foundRight);

    // Kontrolle eines Overflows der Pathlänge, was zu einem unerwarteten
    // Verhalten des Roboters führen kann.
    ...
    // Richtungsänderung in der Pathlänge speichern.
    path[pathLength] = dir;
    pathLength++;
